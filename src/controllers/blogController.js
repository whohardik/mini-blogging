...
const blogDeleteQuery = async function (req, res) {
  try {
    ...
    const authorIdToken = req.authorId;
    ...
    if (data) {
      const allTheBlogs = await blogModel.find(data);

      const filterBlog = allTheBlogs.map((selectedBlog) => {
        if (selectedBlog.authorId.toString() === authorIdToken)
